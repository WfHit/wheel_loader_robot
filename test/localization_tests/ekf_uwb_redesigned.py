...
class AnchorConfig:
    ...
    z_positions: Tuple[float, ...] = (3.0, 6.0)
...
class SimulationConfig:
    ...
    tag_position_z: float = 4.0  # meters height (centered between 3m and 6m)
...
class Visualizer:
    ...
    def create_plots(self, save_path: str = 'ekf_uwb_redesigned.png'):
    ...
    def _plot_3d(self, ax):
        ...
        ax.scatter(anchors[:, 0], anchors[:, 1], anchors[:, 2],
                  c='red', s=50, marker='s', alpha=0.5, label='Anchors')
        ax.plot(traj['positions'][:, 0], traj['positions'][:, 1],
               traj['positions'][:, 2], 'g-', label='Truth', linewidth=2, alpha=0.8)
        ax.plot(ekf['positions'][:, 0], ekf['positions'][:, 1],
               ekf['positions'][:, 2], 'b-', label='EKF', linewidth=1.5, alpha=0.7)

        ax.set_xlabel('X (m)')
        ax.set_ylabel('Y (m)')
        ax.set_zlabel('Z (m)')
        ax.set_title('3D Trajectory\n(Anchors: Y=0m, Z=3m & 6m; Tag: Y≈3m, Z≈4m)',
                     fontweight='bold')
        ax.legend()
    ...
    def _plot_errors(self, ax):
        ...
        ax.plot(uwb_times, x_errors, 'r-', label='X-axis error', linewidth=1.5, alpha=0.8)
        ax.plot(uwb_times, y_errors, 'g-', label='Y-axis error', linewidth=1.5, alpha=0.8)
        ax.plot(uwb_times, z_errors, 'b-', label='Z-axis error', linewidth=1.5, alpha=0.8)
        ax.axhline(10, color='gray', linestyle=':', label='10cm reference', linewidth=1)
        ax.axhline(-10, color='gray', linestyle=':', linewidth=1)
        ax.axhline(0, color='black', linestyle='-', linewidth=0.5, alpha=0.3)

        ax.set_xlabel('Time (s)', fontweight='bold')
        ax.set_ylabel('Position Error (cm)', fontweight='bold')
        ax.set_title('Per-Axis Position Errors Over Time', fontweight='bold')
    ...
    def _plot_axis_errors(self, ax):
        ...
        ax.boxplot([errors[:, 0]*100, errors[:, 1]*100, errors[:, 2]*100],
                  labels=['X', 'Y', 'Z'])
        ax.set_ylabel('Position Error (cm)', fontweight='bold')
        ax.set_title('Per-Axis Error Distribution', fontweight='bold')
    ...
    def _plot_xy(self, ax):
        ...
        ax.scatter(unique_x, np.zeros_like(unique_x), c='red', s=50,
                  marker='s', alpha=0.5, label='Anchors (Y=0m)')
        ax.axhline(0, color='red', linestyle='--', linewidth=1, alpha=0.3)

        ax.plot(traj['positions'][:, 0], traj['positions'][:, 1],
               'g-', label='Truth', linewidth=2, alpha=0.8)
        ax.plot(ekf['positions'][:, 0], ekf['positions'][:, 1],
               'b-', label='EKF', linewidth=1.5, alpha=0.7)

        ax.set_xlabel('X (m)', fontweight='bold')
        ax.set_ylabel('Y (m)', fontweight='bold')
        ax.set_title('XY Trajectory (Top View)\nAnchors at Y=0m, Tag at Y≈3m',
                     fontweight='bold')
        ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    def _plot_xz(self, ax):
        ...
        ax.scatter(anchors[:, 0], anchors[:, 2], c='red', s=30,
                  marker='s', alpha=0.3, label='Anchors (Z=3m & 6m)')

        # Add reference line at tag center Z=4m
        ax.axhline(y=4.0, color='gray', linestyle='--', linewidth=1.5,
                  alpha=0.5, label='Tag center (Z=4m)')
        ...
        # Set explicit Z axis limits to show proper range
        ax.set_ylim(2.5, 6.5)

        ax.set_xlabel('X (m)', fontweight='bold')
        ax.set_ylabel('Z (m)', fontweight='bold')
        ax.set_title('XZ Trajectory (Side View)\nAnchors: Z=3m & 6m, Tag: Z=4m (±0.3m)',
                     fontweight='bold')
        ax.legend()
        ax.grid(True, alpha=0.3)
...
═════════════
• 42 anchors at Y=0m
  Lower row: Z=3m (21 anchors)
  Upper row: Z=6m (21 anchors)
• Vertical baseline: 3m
• Tag: Y=3m, Z=4m
• IMU: 100Hz, UWB: 10Hz

PERFORMANCE (Full-Rate)
══════════════════════
EKF Position:
  RMSE: {metrics['ekf_pos_rmse']*100:.2f} cm
  Mean: {metrics['ekf_pos_mean']*100:.2f} cm
  Max:  {metrics['ekf_pos_max']*100:.2f} cm

Per-Axis RMSE:
  X: {metrics['x_rmse']*100:.2f} cm
  Y: {metrics['y_rmse']*100:.2f} cm
  Z: {metrics['z_rmse']*100:.2f} cm

Velocity: {metrics['vel_rmse']*100:.2f} cm/s

UWB-Rate Performance:
  RMSE: {metrics['ekf_pos_rmse_uwb']*100:.2f} cm

Baseline (UWB-only):
  RMSE: {metrics['baseline_pos_rmse']*100:.2f} cm

Improvement: {((metrics['baseline_pos_rmse']-metrics['ekf_pos_rmse'])/metrics['baseline_pos_rmse']*100):.1f}%
        """
